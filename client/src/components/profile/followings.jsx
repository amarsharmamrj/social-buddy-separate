...
const Followings = (props) => {
    ...
    const fetchFreinds = () => {
        axios.get(`${process.env.REACT_APP_API_SERVICE}/api/users/friends/followings/${props.user._id}`)
            .then((res) => {
    ...
    return (
        <Box className="paddingOneRem">
            <Paper elevation={4} sx={{ borderRadius: "10px" }}>
                <h1 style={{ fontSize: "20px", fontWeight: "700", color: "grey", margin: "0", padding: "10px 0 0 15px" }}>All Followings ({freindsList.length})</h1>
                <List>
                    {
                        ...
                        ) : (<div style={{ padding: "1rem" }}>No followings</div>)
                    }
                </List>
